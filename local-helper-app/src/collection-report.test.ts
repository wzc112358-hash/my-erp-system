...
test('collection report keeps only information selected for follow-up', () => {
  const report = buildCollectionReport(task({
    lastCandidateBundle: {
      source_name: '国能E购',
      candidates: [
        { title: '阻垢剂采购公告', url: '', published_at: '', deadline_at: '', raw_text: '', attachments: [] },
        { title: '办公用品采购公告', url: '', published_at: '', deadline_at: '', raw_text: '', attachments: [] },
      ],
    },
    lastOpportunityCards: [card(), card({ id: 'card-2', title: '办公用品采购公告', recommendedAction: 'ignore' })],
...
test('collection report excludes notices whose Shanghai deadline has passed', () => {
  const report = buildCollectionReport(task({
    lastOpportunityCards: [card({ deadlineAt: '2026-07-07 16:00:00' })],
  }), '2026-07-12T01:00:00.000Z');
...
test('collection report treats a date-only deadline as the end of that Shanghai day', () => {
  const report = buildCollectionReport(task({
    lastOpportunityCards: [card({ deadlineAt: '2026-07-12' })],
  }), '2026-07-12T01:00:00.000Z');
