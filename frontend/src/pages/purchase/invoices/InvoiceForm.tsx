...
export const InvoiceForm: React.FC<InvoiceFormProps> = ({
  ...
  useEffect(() => {
    if (initialValues) {
      const { receive_date, ...rest } = initialValues;
      form.setFieldsValue({
        ...rest,
        receive_date: receive_date ? dayjs(receive_date) : undefined,
      });
  ...
  const validateAmount = (_: unknown, value: number) => {
    ...
    if (selectedContract && selectedContract.uninvoiced_amount !== undefined) {
      if (value > selectedContract.uninvoiced_amount) {
        return Promise.reject(new Error(`发票金额不能超过合同剩余未收票金额 ${selectedContract.uninvoiced_amount.toFixed(2)}`));
      }
    }
    return Promise.resolve();
  };
  ...
  const handleFinish = (values: Record<string, unknown>) => {
    ...
    const data: PurchaseInvoiceFormData = {
      ...
      receive_date: values.receive_date ? (values.receive_date as dayjs.Dayjs).format('YYYY-MM-DD') : '',
    ...
    >
      ...
      {selectedContract && selectedContract.uninvoiced_amount !== undefined && (
        <Row gutter={16}>
          <Col span={24}>
            <Alert
              message={`合同剩余未收票金额: ¥${selectedContract.uninvoiced_amount.toFixed(2)}`}
              type="info"
              showIcon
