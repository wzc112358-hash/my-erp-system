...
const OpportunityMonitorPage: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      successSources: latestRuns.filter((run) => run.status !== 'failed').length,
    };
  }, [items, runs]);
    ...
    <div className="manager-page bid-monitor-page">
      <Flex justify="space-between" align="flex-start" gap={16} wrap className="bid-page-heading">
        <div>
          ...
          <Text type="secondary">每日 08:00 自动巡检公开站点；相同公告不会重复入库，信息保留 30 天。</Text>
        </div>
      ...
      <Row gutter={[12, 12]} className="bid-metric-grid">
        ...
        <Col xs={12} lg={6}><Card><Statistic title="最近成功站点" value={metrics.successSources} suffix={`/ ${sources.length || 10}`} /></Card></Col>
      </Row>
      ...
      <Card className="bid-filter-card">
        <Flex gap={12} wrap>
          ...
          <Select
            ...
            value={source}
            onChange={setSource}
            options={sources.map((item) => ({ label: item.sourceName, value: item.sourceKey }))}
