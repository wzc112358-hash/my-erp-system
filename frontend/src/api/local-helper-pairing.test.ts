...
test('local helper pairing payload stores only pair code hash and expiry', async () => {
  ...
  assert.match(payload.device_fingerprint, /^pending:user-1:[a-f0-9]{16}$/);
  ...
  assert.equal('access_token_hash' in payload, false);
});

test('local helper pairing payload uses a unique pending fingerprint instead of device name', async () => {
  const first = await buildLocalHelperPairingPayload({
    pairCode: 'ABCD1234',
    ownerUser: 'user-1',
    ownerName: '小魏',
    deviceName: '小魏 的 Windows 助手',
  });
  const second = await buildLocalHelperPairingPayload({
    pairCode: 'EFGH5678',
    ownerUser: 'user-1',
    ownerName: '小魏',
    deviceName: '小魏 的 Windows 助手',
  ...
  assert.notEqual(first.device_fingerprint, '小魏 的 Windows 助手');
  assert.notEqual(first.device_fingerprint, second.device_fingerprint);
});

test('local helper pairing payload preserves an explicit fingerprint', async () => {
  const payload = await buildLocalHelperPairingPayload({
    pairCode: 'ABCD1234',
    ownerUser: 'user-1',
    ownerName: '小魏',
    deviceName: '小魏 的 Windows 助手',
    deviceFingerprint: 'explicit-fingerprint',
  });

  assert.equal(payload.device_fingerprint, 'explicit-fingerprint');
});
